...
def run_stats(workdir, outdir, qcdir, mapdir, bismark_methylation_extractor, plotMbias, Rscript):
	...
	with open(outfh, 'w') as fo:
		...
		if bismark_methylation_extractor:
			fo.write('Sample\tRaw_readpairs\tTrimmed_readpairs\tUniq_map_readpairs\tUniq_map_rate\tNo_map_rate\tMulti_map_rate\tDiscarded_seq\tTopS_map\tComp_topS_map\tComp_botS_map\tBotS_map\tMethylated_CpG_rate\tMethylated_CHG_rate\tMethylated_CHH_rate\tMethylated_Cunknown_rate\tDuplication_rate\tFiltered_fragments\tFiltered_Methylated_CpG_rate\tFiltered_Methylated_CHG_rate\tFiltered_Methylated_CHH_rate\tFiltered_CpG_positions\tFiltered_GpC_positions\tGCG_mrate\tGCHH_mrate\tGCHG_mrate\tCCG_mrate\tCCHH_mrate\tCCHG_mrate\tHCG_mrate\tHCHH_mrate\tHCHG_mrate\tHCH_mrate\n')
		else:
			fo.write('Sample\tRaw_readpairs\tTrimmed_readpairs\tUniq_map_readpairs\tUniq_map_rate\tNo_map_rate\tMulti_map_rate\tDuplication_rate\tDiscarded_seq\tTopS_map\tComp_topS_map\tComp_botS_map\tBotS_map\tMethylated_CpG_rate\tMethylated_CHG_rate\tMethylated_CHH_rate\tMethylated_Cunknown_rate\n')
		...
		for sampleid in SampleIDs:
			...
			for samfile in os.listdir(sample_sub_workdir):
				...
				if samfile.endswith("PE_report.txt") or samfile.endswith("SE_report.txt"):
					...
					with open(samfile, 'r') as fh1:
						for l1 in fh1:
							if 'Total number duplicated alignments removed' in l1:
								match = re.search(r'\((\d+\.\d+)%\)', l1)
								duprate = float(match.group(1))
								duprate = f"{duprate:.2f}"
			...
			if bismark_methylation_extractor:
				...
				metrics_ori = (sampleid, rawreadpairs, anaseq, uniqmap, uniqmaprate, nomaprate, multimaprate, duprate, discardseq, topmap, comptopmap, compbotmap, botmap, mCpGrate, mCHGrate, mCHHrate, mCunknownrate)

			metrics = tuple(
					element.replace(',', '').replace('%', '') if isinstance(element, str) else element
					for element in metrics_ori
					)
			pfline = '\t'.join(['%s'] * len(metrics)) % tuple(metrics) + '\n'
			fo.write(pfline)

	print(f"Summary file generated! Check {outfh}")
...
if __name__ == '__main__':
	...
	parser.add_argument('--mapdir', type=str, default='bam', help='Name of the mapping directory')
	parser.add_argument('--outdir', type=str, default='summary', help='Name of the output directory')
	parser.add_argument('--bismark_methylation_extractor', type=str, choices=['True', 'False'], default='False', help='Have run bismark_methylation_extractor or not')
	...
	run_stats(args.workdir, args.outdir, args.qcdir, args.mapdir, args.bismark_methylation_extractor == 'True', args.plotMbias, args.Rscript)